...
```{r}
    ...
    P3
```
that works if all elements of original matrix are positive.
...
```{r}
    ...
    all(diag(P) == 1)
    P = findReversible(Q, afreq = p, method = "AV")
    all(diag(P) == 1)
```
...
```{r}
    ...
    P
```
The two other methods, `AV` and `BA` do not work in this case.
...
```{r}
    ...
    p = c(0.5, 0.5)
    names(p) = 1:2
    m = matrix(c(1 - mu1, mu2, mu1, 1 - mu2), ncol = 2)
    Q = mutationMatrix("custom", matrix = m,  alleles = 1:2, 
                       afreq = p)
    attr(Q, "rate") = expectedMutationRate(Q, afreq = p)
    P1 = findReversible(Q, afreq = p, method = "MH")
    P2 = findReversible(Q, afreq = p, method = "AV")
    P3 = findReversible(Q, afreq = p, method = "BA")
    P3.adj = adjustReversible(Q, P3, afreq = p)
```
